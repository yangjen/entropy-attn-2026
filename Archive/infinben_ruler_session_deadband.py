...
def set_dead_band(model, dead_band: Optional[float]):
    ...
    if layers is not None:
        for layer in layers:
            attn = getattr(layer, "self_attn", None)
            if attn is not None:
                setattr(attn, "dead_band", float(dead_band))
        return
    for m in model.modules():
        if hasattr(m, "num_key_value_groups"):
            setattr(m, "dead_band", float(dead_band))
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Dead band threshold in normalized entropy units. Controller takes no action when |err| < dead_band. Default: disabled.",
    )
    args = ap.parse_args()
    ...
    if args.attn_impl == "entropy_attn":
        ...
        if model_obj is not None:
            ...
            set_dead_band(model_obj, args.dead_band)
