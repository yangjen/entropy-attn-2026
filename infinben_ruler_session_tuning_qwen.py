"""
Ruler:
CUDA_VISIBLE_DEVICES=3 python /c2/jenny/r3/entropy-attn-2026/infinben_ruler_session_tuning_qwen.py \
  --model Qwen/Qwen3.5-2B \
  --dataset_type ruler \
  --data_root /c2/jenny/r3/RULER_outputs/qwen3.5-2b/synthetic/32768/data \
  --tasks qa_2,qa_1 \
  --max_examples 0 \
  --session_size 50 \
  ...
  --metric_mode both \
  --output_root /c2/jenny/r3/RULER_outputs/qwen3.5-2b/synthetic/32768/data_session_runs \
  ...
  --run_tag 50session_3cali_prefixON.jsonl

Baseline (entropy_attn fixed-temp):
CUDA_VISIBLE_DEVICES=0 python /c2/jenny/r3/entropy-attn-2026/infinben_ruler_session_tuning_qwen.py \
  --model Qwen/Qwen3.5-2B \
  --dataset_type ruler \
  --data_root /c2/jenny/r3/RULER_outputs/qwen3.5-2b/synthetic/32768/data \
  --tasks qa_2,qa_1 \
...
InfiniteBench (HuggingFace, session tuning):
CUDA_VISIBLE_DEVICES=0 python /c2/jenny/r3/entropy-attn-2026/infinben_ruler_session_tuning_qwen.py \
  --model Qwen/Qwen3.5-2B \
  --dataset_type infinitebench \
  --infinitebench_source hf \
  --tasks code_run \
  --max_examples 0 \
  --filter_max_tokens 250000 \
  --session_size 50 \
  --session_init_mode calibrated_per_head \
  --session_calibration_samples 3 \
  --session_target_stat mean \
  ...
  --max_step 0.0005 \
  --target_trim_ratio 0.10 \
  --prompt_style infinitebench \
  --output_root /c2/jenny/r3/InfiniteBench_outputs/qwen3.5-2b/data_session_runs \
  --attn_impl entropy_attn \
  --dtype bf16 \
  --compact \
  --max_new_tokens 16 \
  --deterministic \
  --time --time_skip 4 \
  --run_tag 50session_3cali_250kfilter.jsonl

InfiniteBench (local subset, e.g. 64k-131k bucket):
CUDA_VISIBLE_DEVICES=1 python /c2/jenny/r3/entropy-attn-2026/infinben_ruler_session_tuning_qwen.py \
  --model Qwen/Qwen2.5-3B-Instruct \
  --dataset_type infinitebench \
  --infinitebench_source local_subset \
  --data_root /c2/jenny/r3/InfiniteBench_subsets \
  --infinitebench_bucket 64k_131k \
  --tasks longbook_qa_eng,kv_retrieval \
  --max_examples 0 \
  --session_size 50 \
  --session_init_mode calibrated_per_head \
  --session_calibration_samples 3 \
  --prompt_style infinitebench \
  --output_root /c2/jenny/r3/InfiniteBench_outputs/qwen/data_session_runs \
  --attn_impl entropy_attn \
  --dtype bf16 \
  --compact \
  --max_new_tokens 32 \
  ...
  --deterministic \
  --run_tag 50session_3cali_64k131k.jsonl
"""
...
def aggregate_target_tensors(tensors: List[torch.Tensor], stat: str) -> torch.Tensor:
    ...
    if stat == "mean":
        out = xs.mean(dim=0)  # [Z, H, 1]
    else:
        out = xs.median(dim=0).values  # [Z, H, 1]
    # Ensure Z=1 without adding extra dims — prompt_target_entropy is (Z, H, 1)
    if out.shape[0] != 1:
...
def filter_by_token_length(
    ...
    dataset_type: str,
    prompt_style: str,
    ruler_append_answer_prefix: bool,
    max_tokens: int,
    min_tokens: int,
):
    for ex in examples:
        ex_u = adapt_example(ex, dataset_type, ruler_append_answer_prefix=ruler_append_answer_prefix)
        prompt = build_prompt(runner, ex_u, prompt_style)
        n = len(runner.tokenizer(prompt, add_special_tokens=False)["input_ids"])
        if max_tokens > 0 and n > max_tokens:
            continue
        if min_tokens > 0 and n < min_tokens:
            continue
        yield ex
...
def ruler_hit_all(pred: str, ref_list: List[str]) -> float:
    """string_match_all: fraction of references found in prediction (matches RULER paper metric for cwe/fwe)."""
    if not ref_list:
        return 0.0
    pred_l = (pred or "").lower()
    return sum(1.0 for r in ref_list if (r or "").lower() in pred_l) / len(ref_list)


# --- Official InfiniteBench scorers (from compute_scores.py) ---
...
def _ib_f1_score(prediction: List[str], ground_truth: List[str]) -> float:
...
def _ib_qa_f1_score(pred: str, ground_truths: List[str]) -> float:
    f1 = 0.0
    for gt in ground_truths:
        p_toks = _ib_normalize_answer(pred).split()
        g_toks = _ib_normalize_answer(gt).split()
        if p_toks and g_toks:
            f1 = max(f1, _ib_f1_score(p_toks, g_toks))
    return f1


def _ib_score_kv_retrieval(pred: str, label, **_) -> float:
    if isinstance(label, list):
        label = label[0]
    label = str(label)
    for c in ["\n", ":", '"', "'", ".", ",", "?", "!", "{", "}"]:
        pred = pred.replace(c, " ")
    return float(label in pred.split())


def _ib_score_passkey(pred: str, label, **_) -> float:
    if isinstance(label, list):
        label = label[0]
    pred_list = re.split("[^0-9]", pred)
    for item in pred_list:
        if item:
            return float(str(label) == item)
    return 0.0


def _ib_score_number_string(pred: str, label, **_) -> float:
    return _ib_score_passkey(pred, label)


def _ib_score_code_run(pred: str, label, **_) -> float:
    if isinstance(label, list):
        label = label[0]
    try:
        label = int(label)
    ...
    if not words:
        return 0.0
    try:
        return float(label == int(words[-1]))
    except Exception:
        return 0.0


def _ib_score_longdialogue_qa_eng(pred: str, label, **_) -> float:
    pred = pred.strip().upper()
    for item in (label if isinstance(label, list) else [label]):
        if str(item).upper() in pred:
            return 1.0
    return 0.0


def _ib_score_longbook_qa_eng(pred: str, label, **_) -> float:
    return _ib_qa_f1_score(pred, label if isinstance(label, list) else [label])


def _ib_score_longbook_choice_eng(pred: str, label, **_) -> float:
    pred = pred.strip()
    match = re.search(r"\b[A-D]\b(?!.*\b[A-D]\b)", pred)
    if match and match.group(0) in label:
        return 1.0
    if pred and pred[0] in "ABCD" and pred[0] in label:
        return 1.0
    if pred in label:
        return 1.0
    for c in ["\n", '"', "'", ".", ",", "?", "!", "{", "}"]:
    ...
    for word in pred.split():
        if word in "ABCD":
            return float(word in label)
    return 0.0


def _ib_score_math_calc(pred: str, label, **_) -> float:
    if isinstance(label, list) and label and isinstance(label[0], list):
        label = label[0]
    try:
        label_ints = [int(x) for x in label]
    except (ValueError, TypeError):
        return 0.0
    pred_nums = [int(x) for x in re.split("[^0-9]", pred) if x]
    cnt = 0
    for i, lv in enumerate(label_ints):
        if i >= len(pred_nums):
            break
        if lv == pred_nums[i]:
            cnt += 1
            ...
            break
    return cnt / len(label_ints) if label_ints else 0.0


_INFINITEBENCH_SCORERS: Dict[str, Any] = {
    "kv_retrieval": _ib_score_kv_retrieval,
    "kv_retrieval_prefix": _ib_score_kv_retrieval,
    "kv_retrieval_both": _ib_score_kv_retrieval,
    "passkey": _ib_score_passkey,
    "number_string": _ib_score_number_string,
    "code_run": _ib_score_code_run,
    "longdialogue_qa_eng": _ib_score_longdialogue_qa_eng,
    "longbook_qa_eng": _ib_score_longbook_qa_eng,
    "longbook_choice_eng": _ib_score_longbook_choice_eng,
    "math_calc": _ib_score_math_calc,
}
...
def compact_row_common(
    ...
    if ib_score is not None:
        row["_ib_score"] = round(ib_score, 4)
    else:
        row["_contains_hit"] = bool(c_hit)
        row["_exact_hit"] = bool(e_hit)
        row["_ruler_part_hit"] = bool(r_hit)
        row["_ruler_all_hit"] = round(ra_hit, 4)
    return row


def build_prompt(runner: QwenRunner, ex: Dict[str, Any], style: str) -> str:
    if style == "ruler_raw":
        raw = ex.get("_raw", ex)
    ...
    return runner.messages_to_prompt(messages)
...
def task_examples_from_hf(task: str):
    # Try direct file download first to avoid fsspec glob issues seen with some datasets versions.
    ...
    if load_dataset is None:
        raise RuntimeError(
            "Could not download InfiniteBench task; install `datasets` or `huggingface_hub`, "
            "or use --infinitebench_source local_jsonl / local_subset with --data_root."
        )
    ...
    if task not in ds:
        raise KeyError(f"Task '{task}' not found in InfiniteBench HF dataset.")
    return ds[task]


def task_examples_from_local_jsonl(task: str, data_root: str):
    path = os.path.join(data_root, f"{task}.jsonl")
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing InfiniteBench file: {path}")
    return iter_jsonl(path)


def task_examples_from_local_subset(task: str, data_root: str, bucket: Optional[str] = None):
    import glob as _glob
    task_dir = os.path.join(data_root, task)
    if not os.path.isdir(task_dir):
        raise FileNotFoundError(f"Missing InfiniteBench subset dir: {task_dir}")
    if bucket:
        fname = bucket if bucket.endswith(".jsonl") else f"{bucket}.jsonl"
        path = os.path.join(task_dir, fname)
        if not os.path.exists(path):
            raise FileNotFoundError(f"Missing bucket file: {path}")
        return iter_jsonl(path)

    # No bucket specified: concatenate all non-empty bucket files, sorted by name.
    files = sorted(_glob.glob(os.path.join(task_dir, "*.jsonl")))
...
def main():
    ...
    ap.add_argument("--tasks", default="qa_2")
    ap.add_argument(
        "--data_root",
        default=None,
        help=(
            "RULER: <data_root>/<task>/validation.jsonl. "
            "InfiniteBench local_jsonl: <data_root>/<task>.jsonl. "
            "InfiniteBench local_subset: <data_root>/<task>/<bucket>.jsonl."
        ),
    )
    ap.add_argument(
        "--infinitebench_source",
        choices=["hf", "local_jsonl", "local_subset"],
        default="hf",
        help="InfiniteBench data source (only used when --dataset_type infinitebench).",
    )
    ap.add_argument(
        "--infinitebench_bucket",
        default=None,
        help="Subset bucket name, e.g. 64k_131k (for --infinitebench_source local_subset). "
             "Omit to concatenate all non-empty bucket files.",
    )
    ap.add_argument(
        "--output_root",
        default="/c2/jenny/r3/RULER_outputs/qwen3.5-2b/synthetic/32768/data_session_runs",
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="Silently skip examples whose prompt token count exceeds this value. 0 = disabled.",
    )
    ap.add_argument(
        "--filter_min_tokens",
        type=int,
        default=0,
        help="Silently skip examples whose prompt token count is below this value. 0 = disabled.",
    )
    ap.add_argument(
        "--overlength_policy",
    ...
    ap.add_argument("--prompt_style", choices=["infinitebench", "minimal", "ruler_raw"], default="ruler_raw")
    ap.add_argument(
        "--ruler_append_answer_prefix",
        action="store_true",
    )
    ...
    ap.add_argument("--run_tag", default="qwen_predictions.jsonl")
    ...
    ap.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable Qwen3 thinking mode. Off by default: thinking tokens burn max_new_tokens budget.",
    )
    ...
    ap.add_argument("--session_size", type=int, default=100000)
    ap.add_argument(
        "--session_init_mode",
    ...
    ap.add_argument("--session_temp_target_gain", type=float, default=0.0)
    ...
    if args.dataset_type == "ruler" and not args.data_root:
        raise ValueError("--data_root is required when --dataset_type ruler")
    if args.dataset_type == "infinitebench" and args.infinitebench_source != "hf" and not args.data_root:
        raise ValueError("--data_root is required when --infinitebench_source is local_jsonl or local_subset")
    ...
    for task in tasks:
        ...
        hit_ruler_all = 0
        hit_official = 0.0
        use_ib_scorer = args.dataset_type == "infinitebench" and task in _INFINITEBENCH_SCORERS
        session_summaries: List[Dict[str, Any]] = []

        if args.dataset_type == "ruler":
            examples = task_examples_from_ruler(task, args.data_root)
        elif args.infinitebench_source == "local_jsonl":
            examples = task_examples_from_local_jsonl(task, args.data_root)
        elif args.infinitebench_source == "local_subset":
            examples = task_examples_from_local_subset(task, args.data_root, args.infinitebench_bucket)
        else:
            examples = task_examples_from_hf(task)

        if args.filter_max_tokens > 0 or args.filter_min_tokens > 0:
            examples = filter_by_token_length(
                examples,
                runner,
                args.dataset_type,
                args.prompt_style,
                args.ruler_append_answer_prefix,
                max_tokens=args.filter_max_tokens,
                min_tokens=args.filter_min_tokens,
            )
        ...
        with open(pred_path, "w", encoding="utf-8") as out_f, entropy_ctx as ent_f:
            ...
            for session_idx, session_examples in enumerate(sessions):
                ...
                for ex in session_examples:
                    ...
                    if use_ib_scorer:
                        ib_score = float(_INFINITEBENCH_SCORERS[task](pred, list(golds)))
                        hit_official += ib_score
                        session_hit_official += ib_score
                        r_hit = ra_hit = c_hit = e_hit = 0
                    else:
                        ib_score = None
                    ...
                    if args.compact:
                        ...
                        if use_ib_scorer:
                            ex_out["_ib_score"] = round(ib_score, 4)
                        else:
                            ex_out["_contains_hit"] = bool(c_hit)
                            ex_out["_exact_hit"] = bool(e_hit)
                            ex_out["_ruler_part_hit"] = bool(r_hit)
                            ex_out["_ruler_all_hit"] = round(ra_hit, 4)
                    out_f.write(json.dumps(ex_out, ensure_ascii=False) + "\n")

                    if args.status_every > 0 and (total % args.status_every == 0):
                        if use_ib_scorer:
                            print(
                                f"[{task}] {total} done | {task}_acc={100.0 * hit_official / total:.2f}%"
                                f" ({hit_official:.1f}/{total})"
                            )
                        else:
                ...
                if use_ib_scorer:
                    session_summary["ib_acc"] = round(100.0 * session_hit_official / max(session_n, 1), 2)
                else:
                    session_summary["contains_acc"] = round(100.0 * session_hit_contains / max(session_n, 1), 2)
                    session_summary["exact_acc"] = round(100.0 * session_hit_exact / max(session_n, 1), 2)
                    session_summary["ruler_part_acc"] = round(100.0 * session_hit_ruler / max(session_n, 1), 2)
                    session_summary["ruler_all_acc"] = round(100.0 * session_hit_ruler_all / max(session_n, 1), 2)
                if args.attn_impl == "entropy_attn":
                ...
                if use_ib_scorer:
                    print(
                        ...
                        f"{task}_acc={session_summary['ib_acc']:.2f}%"
                    )
                else:
                    print(
                        f"[{task}] session {session_idx} | n={session_n} | "
        ...
        if use_ib_scorer:
            task_summary["ib_acc"] = round(100.0 * hit_official / max(total, 1), 2)
        else:
            task_summary["contains_acc"] = round(100.0 * hit_contains / max(total, 1), 2)
            task_summary["exact_acc"] = round(100.0 * hit_exact / max(total, 1), 2)
            task_summary["ruler_part_acc"] = round(100.0 * hit_ruler / max(total, 1), 2)
            task_summary["ruler_all_acc"] = round(100.0 * hit_ruler_all / max(total, 1), 2)
        if args.time:
            task_summary["pred_time"] = _summarize_times(pred_times_s)
        summary[task] = task_summary

    final_report = {"config": vars(args), "summary": summary}
    summary_prefix = "ruler_summary" if args.dataset_type == "ruler" else "infinitebench_summary"
    summary_path = os.path.join(out_dir, f"{summary_prefix}_{args.attn_impl}_{args.run_tag}.json")
